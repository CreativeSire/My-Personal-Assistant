...
from sdk import get_config, emit_event, system_metrics
...
class OpsHealthCheckSkill(Skill):
    ...
    def tool_ops_health_summary(self) -> str:
        ...
        try:
            emit_event(
                event_type="skill.ops_health.summary",
                actor="ops_health_check",
                payload={"cpu": metrics.get("cpu", 0), "ram": metrics.get("ram", 0), "disk": metrics.get("disk", 0), "queue_depth": depth},
                channel="system",
                risk_score=0.1,
            )
        except Exception:
