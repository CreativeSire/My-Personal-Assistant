...
class ProactiveEngine:
    ...
    def __init__(self):
        ...
        # Conservative defaults (must be defined before _sent_events deque)
        self._cooldown_seconds = 300
        self._dup_window_seconds = 3600
        self._max_notifications_per_hour = 12
        # Use a bounded deque instead of a list to prevent O(n) rebuild and memory growth.
        self._sent_events: collections.deque = collections.deque(maxlen=self._max_notifications_per_hour)
        ...
        for item in checks or []:
            ...
            self._checks.append(
                ProactiveCheck(
                    ...
                    user_id=item.get("user_id") or self._cfg.default_owner_user_id,
                )
            )
    ...
    def _can_send(self, message: str) -> bool:
        now = time.time()
        with self._lock:
            # Count only events within the rolling 1-hour window.
            recent_count = sum(1 for t in self._sent_events if now - t < 3600)
            if recent_count >= self._max_notifications_per_hour:
                return False
            ...
            # deque is bounded — oldest entry is auto-evicted when full.
        ...
        for check in self._checks:
            ...
            try:
                ...
                if output:
                    ...
                    user_id = check.user_id or self._cfg.default_owner_user_id
