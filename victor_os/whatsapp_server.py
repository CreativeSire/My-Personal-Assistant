...
_wa_adapter = WhatsAppAdapter(tier_router=_router)
...
def whatsapp_reply():
    ...
    inbound = _wa_adapter.normalize_inbound(request)
    incoming_msg = inbound.text
    ...
    decision = _wa_adapter.route(inbound)
    tier = decision.tier
    if tier == "SOCIAL":
        resp = MessagingResponse()
        msg = resp.message()
        fast = get_fast_response(incoming_msg)
        _wa_adapter.deliver(
            OutboundEnvelope(
                channel="whatsapp",
                destination=sender,
                text=fast,
                session_id=resolve_session_id("whatsapp", sender or "unknown"),
                idempotency_key=f"social:{inbound.message_id}",
            )
        )
        msg.body(fast)
        return str(resp)
