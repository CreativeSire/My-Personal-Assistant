...
const controlTaskForm = document.getElementById("controlTaskForm");
const controlTaskInput = document.getElementById("controlTaskInput");
const controlTaskOut = document.getElementById("controlTaskOut");
const activityFeed = document.getElementById("activityFeed");
const goalsFeed = document.getElementById("goalsFeed");
const memorySearchForm = document.getElementById("memorySearchForm");
const memorySearchInput = document.getElementById("memorySearchInput");
const memorySearchOut = document.getElementById("memorySearchOut");
...
async function refreshActivity() {
  ...
  try {
    ...
    activityFeed.innerHTML = events.slice(0, 50).map((e) => {
      const ts = new Date(Number(e.ts_utc || 0) * 1000).toLocaleString();
      const type = escapeHtml(String(e.event_type || ""));
      const actor = escapeHtml(String(e.actor || ""));
      return `<div class="terminal-line info">[${escapeHtml(ts)}] ${type} (${actor})</div>`;
    }).join("") || `<div class="terminal-line">No events yet.</div>`;
  } catch (err) {
    activityFeed.innerHTML = `<div class="terminal-line warn">Activity unavailable: ${escapeHtml(err.message)}</div>`;
  }
}
...
async function refreshGoals() {
  ...
  try {
    ...
    goalsFeed.innerHTML = goals.slice(0, 50).map((g) => {
      const p = Number(g.priority || 0);
      const prog = Number(g.progress || 0);
      return `<div class="terminal-line">[P${p}] ${escapeHtml(g.status || "")} ${escapeHtml(g.title || "")} (${prog}%)</div>`;
    }).join("") || `<div class="terminal-line">No goals.</div>`;
  } catch (err) {
    goalsFeed.innerHTML = `<div class="terminal-line warn">Goals unavailable: ${escapeHtml(err.message)}</div>`;
  }
}

async function runControlTask(text) {
  if (!controlTaskOut) return;
  controlTaskOut.textContent = "Queuing...";
  try {
    const data = await apiPost("/api/control/tasks", { text });
    if (!data.ok) {
      controlTaskOut.textContent = `Failed: ${JSON.stringify(data.body || data.error || {})}`;
      return;
    }
    const body = data.body || {};
    const taskId = body.task_id || (body.task && body.task.id) || "";
    controlTaskOut.textContent = taskId ? `Queued: ${taskId}` : "Queued.";
  } catch (err) {
    controlTaskOut.textContent = `Failed: ${err.message}`;
  }
}

if (controlTaskForm && controlTaskInput) {
  controlTaskForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const text = String(controlTaskInput.value || "").trim();
    if (!text) return;
    controlTaskInput.value = "";
    await runControlTask(text);
  });
}

if (memorySearchForm && memorySearchInput && memorySearchOut) {
  memorySearchForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    const q = String(memorySearchInput.value || "").trim();
    if (!q) return;
    memorySearchOut.innerHTML = `<div class="terminal-line">Searching...</div>`;
    try {
      const data = await apiGet(`/api/memory/search?q=${encodeURIComponent(q)}`);
      memorySearchOut.innerHTML = `<div class="terminal-line info">${escapeHtml(JSON.stringify(data.results || {}, null, 2))}</div>`;
    } catch (err) {
      memorySearchOut.innerHTML = `<div class="terminal-line warn">${escapeHtml(err.message)}</div>`;
    }
  });
}
