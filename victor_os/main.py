...
def _run_turn(
    ...
    # --- CRITIC LAYER ---
    if (not skip_critic) and cfg.critic_enabled:
        try:
...
def run_assistant():
    ...
    try:
        voice = None
        if is_voice_mode and getattr(cfg, "voice_enabled", False):
            voice = VoiceInterface(on_transcript=lambda text: _run_turn(text, user_id=user_id, session_id=session_id), wake_word=getattr(cfg, "wake_word", "jarvis"))
            voice.start()
        while running:
            user_input = ""
            if is_voice_mode:
                if voice is None:
                    ...
                    # Wake-word mode runs in background; CLI input acts as emergency stop.
                    user_input = input("\n(voice) Type 'exit' to quit: ").strip()
            else:
                user_input = input("\nYou: ").strip()
            ...
            try:
                tier = _router.classify_tier(user_input)
                if tier == "SOCIAL":
                    response = get_fast_response(user_input)
                elif tier == "SIMPLE":
                    response = _run_turn(
                        user_input,
                        user_id=user_id,
        ...
        try:
            if is_voice_mode and getattr(cfg, "voice_enabled", False) and voice is not None:
                voice.stop()
        except Exception:
            pass
        queue.stop_worker()
