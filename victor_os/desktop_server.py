...
_desktop_router = LocalInferenceRouter()
_desktop_adapter = DesktopAdapter(tier_router=_desktop_router)
...
def _victor_api_base() -> str:
    return (cfg.victor_task_api_url or "http://127.0.0.1:8787").rstrip("/")


def _victor_api_headers() -> dict[str, str]:
    headers = {"Content-Type": "application/json"}
    if cfg.victor_api_key:
        headers["X-API-Key"] = cfg.victor_api_key
    return headers


def _victor_api_request(method: str, path: str, payload: dict[str, Any] | None = None) -> tuple[int, dict[str, Any]]:
    url = f"{_victor_api_base()}{path}"
    data = None
    if payload is not None:
        data = json.dumps(payload).encode("utf-8")
    req = urllib.request.Request(url, data=data, method=method.upper(), headers=_victor_api_headers())
    try:
        ...
        raw = exc.read().decode("utf-8") if getattr(exc, "fp", None) else "{}"
        try:
...
def api_control_health():
    status, body = _victor_api_request("GET", "/v1/capabilities", None)
    return jsonify({"ok": status == 200, "status_code": status, "body": body, "base_url": _victor_api_base()}), (200 if status == 200 else 502)


@app.get("/api/control/tasks/latest")
def api_control_tasks_latest():
    limit = int(request.args.get("limit", "10") or "10")
    limit = max(1, min(50, limit))
    status, body = _victor_api_request("GET", f"/v1/tasks/latest?limit={limit}", None)
    return jsonify({"ok": status == 200, "status_code": status, "body": body}), (200 if status == 200 else 502)


@app.post("/api/control/orchestrate/run")
def api_control_orchestrate_run():
    payload = request.get_json(silent=True) or {}
    status, body = _victor_api_request("POST", "/v1/orchestrate/run", payload)
    return jsonify({"ok": status == 200, "status_code": status, "body": body}), (200 if status == 200 else 502)


@app.post("/api/control/feedback")
def api_control_feedback():
    payload = request.get_json(silent=True) or {}
    status, body = _victor_api_request("POST", "/v1/feedback", payload)
    return jsonify({"ok": status == 200, "status_code": status, "body": body}), (200 if status == 200 else 502)


@app.post("/api/control/training/export")
def api_control_training_export():
    payload = request.get_json(silent=True) or {}
    status, body = _victor_api_request("POST", "/v1/training/export", payload)
    return jsonify({"ok": status == 200, "status_code": status, "body": body}), (200 if status == 200 else 502)
...
@app.get("/api/routes")
def api_routes():
    # Debug helper: lets us verify route registration in live pilot.
    try:
        rules = sorted([str(r) for r in app.url_map.iter_rules()])
        return jsonify({"ok": True, "routes": rules})
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc), "routes": []}), 500
...
def api_activity():
    ...
    try:
        events = _data_engine.list_recent_events(limit=50)
        # Trim large payloads
        for e in events:
...
def api_goals():
    try:
        ...
        gt = get_goal_tracker()
        goals = gt.list_goals(status_filter=None)
        out = []
        for g in goals:
            out.append(
                {
        ...
        return jsonify({"ok": True, "goals": out})
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc), "goals": []}), 500


@app.post("/api/control/tasks")
def api_control_create_task():
    payload = request.get_json(silent=True) or {}
    inbound = _desktop_adapter.normalize_inbound(payload)
    text = inbound.text
    if not text:
        return jsonify({"ok": False, "error": "text is required"}), 400
    # Bridge to Victor control plane task API.
    status, body = _victor_api_request(
        "POST",
        "/v1/tasks",
        {
            "intent": text,
...
def api_memory_search():
    ...
    try:
        from memory_core import MemoryQueryInput, recall_memory

        res = recall_memory(MemoryQueryInput(query=q, top_k=10))
...
def api_task_cancel(task_id: str):
    try:
        ...
        return jsonify({"ok": bool(ok), "task_id": task_id, "state": "cancelled" if ok else "unchanged"}), (200 if ok else 409)
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.post("/api/tasks/<task_id>/approve")
def api_task_approve(task_id: str):
    payload = request.get_json(silent=True) or {}
    scope = str(payload.get("scope") or "single_action")
    return jsonify({"ok": True, "task_id": task_id, "approved": True, "scope": scope})


@app.get("/api/tasks/<task_id>/artifacts")
def api_task_artifacts(task_id: str):
    job_root = os.path.join(cfg.workspace_dir, "jobs", task_id)
    items: list[dict[str, Any]] = []
    if os.path.isdir(job_root):
        for root, _, files in os.walk(job_root):
            for name in files:
                path = os.path.join(root, name)
                rel = os.path.relpath(path, cfg.base_dir).replace("\\", "/")
                items.append({"path": rel, "size": os.path.getsize(path)})
    return jsonify({"ok": True, "task_id": task_id, "count": len(items), "artifacts": items[:300]})
...
def api_chat():
    ...
    raw_user = inbound.sender_id or str(payload.get("user_id") or "desktop_ceejay")
    ...
    try:
        user_id = resolve_user_id("desktop", raw_user)
        session_id = requested_session_id or resolve_session_id("desktop", raw_user)
        route_decision = _desktop_adapter.route(inbound)
        _emit_platform_event(
            "intent.received",
            session_id=session_id,
            actor=user_id,
            payload={"intent": message, "channel": "desktop", "tier": route_decision.tier, "intent_class": route_decision.intent_class},
            risk_score=0.2,
        )
        if sleeping_agents:
            message = (
        ...
        active_agent = active_agents[-1] if active_agents else "Chief_of_Staff"
        _desktop_adapter.deliver(
            OutboundEnvelope(
                channel="desktop",
                destination=raw_user,
                text=(clean or response.strip()),
                session_id=session_id,
                idempotency_key=f"chat:{inbound.message_id}",
            )
        )
        _emit_platform_event(
            "task.completed",
