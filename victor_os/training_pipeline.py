...
def _count_redaction_hits(value: str) -> int:
    text = str(value or "")
    hits = 0
...
class TrainingPipeline:
    ...
    def export_dataset(self, out_dir: str, eval_ratio: float = 0.2, seed: int = 42) -> DatasetExportResult:
        ...
        redaction_hits = 0
        seen = set()
        min_input_len = int(os.getenv("TRAINING_MIN_INPUT_LEN", "8"))
        for e in events:
            payload = e.get("payload_json") or {}
            raw_payload = json.dumps(payload, ensure_ascii=False)
            if len(raw_payload.strip()) < min_input_len:
                continue
            key = f"{e.get('event_type')}::{raw_payload}"
            if key in seen:
                continue
            seen.add(key)
            redaction_hits += _count_redaction_hits(raw_payload)
            examples.append(
                {
        ...
        try:
            ...
            for row in rows:
                rejected = str(row["rejected_output"] or "")
                preferred = str(row["preferred_output"] or "")
                if len(rejected.strip()) < min_input_len and len(preferred.strip()) < min_input_len:
                    continue
                ckey = f"{row['reason_code']}::{rejected}::{preferred}"
                if ckey in correction_seen:
                    continue
                correction_seen.add(ckey)
                corrections.append(
                    {
                        ...
                        "input": redact_text(rejected),
                        "target": redact_text(preferred),
                        "ts_utc": row["ts_utc"],
                    }
                )
                redaction_hits += _count_redaction_hits(rejected)
                redaction_hits += _count_redaction_hits(preferred)
        finally:
            conn.close()
        ...
        manifest = {
            "schema_version": "phase3.dataset.v1",
            ...
            "hashes": {
                "train_sha256": _sha256_file(train_path) if os.path.exists(train_path) else "",
                "eval_sha256": _sha256_file(eval_path) if os.path.exists(eval_path) else "",
                "golden_sha256": _sha256_file(golden_path) if os.path.exists(golden_path) else "",
            },
        }
    ...
    def promote_checkpoint(
        ...
        model_track: str = "routine_policy_model",
        ...
        allowed = (
            ...
            and bool(safety_gate_passed)
            and float(regression_delta) >= 0.0
        )
        ...
        event = {
            ...
            "accuracy": accuracy,
            "allowed": allowed,
            "training_manifest_hash": str(training_manifest_hash or ""),
